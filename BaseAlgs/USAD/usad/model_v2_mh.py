...
def freeze(layer, freeze_index_list):
    for layer_index, child in enumerate(layer.children()):
        if layer_index in freeze_index_list:
            # print(f"{layer_index}冻结{child}")
            for param in child.parameters():
                param.requires_grad = False
        # else:
        #     print(f"{layer_index}未冻结{child}")
...
class USAD:
    ...
    def __init__(self, x_dims: int, max_epochs: int = 250, batch_size: int = 128,
        ...
        self._step = 0
        self.require_improvement = require_improvement # 允许验证集loss连续变大的epoch最大数量

        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # 加参数，表示是否冻结编码器
        self._shared_encoder = Encoder(input_dims=self._input_dims, z_dims=self._z_dims, freeze_index_list=freeze_index_list_encoder)
        self._decoder_G = Decoder(z_dims=self._z_dims, input_dims=self._input_dims, freeze_index_list=freeze_index_list_decoder)
        self._decoder_D = Decoder(z_dims=self._z_dims, input_dims=self._input_dims, freeze_index_list=freeze_index_list_decoder)
    ...
    def compose_loss(self, w, w_G, w_G_D, w_D, epoch, loss_type='train'):
        ...
        mse_left_G = self.loss_func(w_G, w).cuda() if self.device == torch.device('cuda') else self.loss_func(w_G, w)
        ...
        mse_right_G = self.loss_func(w_G_D, w).cuda() if self.device == torch.device('cuda') else self.loss_func(w_G_D, w)
        ...
        mse_left_D = self.loss_func(w_D, w).cuda() if self.device == torch.device('cuda') else self.loss_func(w_D, w)
        ...
        mse_right_D = self.loss_func(w_G_D, w).cuda() if self.device == torch.device('cuda') else self.loss_func(w_G_D, w)
    ...
    def fit(self, values, model_save_dir: pathlib.Path, valid_portion=0.3, local_epoch=None, if_finetune=False):
        ...
        # all_data = []
        # for data in train_sliding_window:
        #     all_data.append(data)
        # all_data = torch.tensor([item.cpu().detach().numpy() for item in train_sliding_window ]).cuda()
        ...
        # all_data = torch.tensor([item.cpu().detach().numpy() for item in all_data ]).cuda()
        train_data = np.array(all_data)[train_index_list]
        valid_data = np.array(all_data)[valid_index_list]
        # train_data = all_data.cpu().numpy()[train_index_list]
        # valid_data = all_data.cpu().numpy()[valid_index_list]
        ...
        total_time = 0
        max_epoch = local_epoch if local_epoch is not None else self._max_epochs
            ...
            for i, w in enumerate(train_data):
                ...
                if self._step > 0 and self._step % self._valid_step_freq == 0:
                    for w in valid_data:
                        ...
                        # w_D = self._decoder_D(z).cuda() if self.device == torch.device('cuda') else self._decoder_D(z)
                        w_G_D = self._decoder_D(self._shared_encoder(w_G)).cuda() if self.device == torch.device('cuda') else self._decoder_D(self._shared_encoder(w_G))
                        # loss_G, loss_D = self.compose_loss(w, w_G, w_G_D, w_D, epoch, 'valid')
                        self.valid_value[-1] += torch.sum(self.loss_func(w, w_G_D)).cpu().detach().item()
                    # if self.loss['AE_G']['valid'][-1] - self.loss['AE_D']['valid'][-1] > max_delta_loss_gd:
                    #     max_delta_loss_gd = self.loss['AE_G']['valid'][-1] - self.loss['AE_D']['valid'][-1]
    ...
    def predict(self, values, alpha=0.5, beta=0.5, on_dim=False):
        ...
        for w in test_sliding_window:
            ...
            w_G = self._decoder_G(z).cuda() if self.device == torch.device('cuda') else self._decoder_G(z)
            w_G_D = self._decoder_D(self._shared_encoder(w_G)).cuda() if self.device == torch.device('cuda') else self._decoder_D(self._shared_encoder(w_G))
            batch_scores_d = self.loss_func(w_G, w).cuda() if self.device == torch.device('cuda') else self.loss_func(w_G, w)
            batch_scores_g = self.loss_func(w_G_D, w).cuda() if self.device == torch.device('cuda') else self.loss_func(w_G_D, w)
            ...
            batch_scores_d = batch_scores_d.cuda().data.cpu().numpy() if self.device == torch.device('cuda') else batch_scores_d.data.numpy()
            batch_scores_g = batch_scores_g.cuda().data.cpu().numpy() if self.device == torch.device('cuda') else batch_scores_g.data.numpy()
    ...
    def save(self, model_save_dir):
        ...
        print('save model to ',shared_encoder_path)
